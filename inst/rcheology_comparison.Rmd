...
Many of the extra functions appear in `defunct` Rd files. It may be possible to use heuristics to strip those out. It may be that by parsing the `NAMESPACE` file of a package we'd be able to catch that some of these aren't exported.
...
Ideas to improve:
 ...
 - Use `alias` to at least discover the existence of functions/objects, even if we don't know their signature.
